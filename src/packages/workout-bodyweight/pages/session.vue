...
const currentMediaSource = computed(() => {
  ...
  return `/packages/workout-bodyweight/static/${folder}/${exercise.id}.${extension}`
})
...
function finish() {
  ...
  if (completed) uni.redirectTo({ url: `/packages/workout-bodyweight/pages/complete?sessionId=${completed.id}` })
}
...
function handleMediaError(event: unknown) {
  ...
  console.warn('[exercise-media] bodyweight animation failed', exercise.id, event)
